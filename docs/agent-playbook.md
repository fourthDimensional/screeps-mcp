...
3. For normal local bot work, call `validate_files` then `deploy_files_and_verify` on the intended branch. It records provenance and a baseline, reads the remote modules back to confirm their hash, and never activates a branch implicitly. Use `deploy_and_verify` when a complete manifest is already available.
